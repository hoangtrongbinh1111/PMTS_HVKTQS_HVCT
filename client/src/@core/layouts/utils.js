...
export const canViewMenuGroup = item => {
  ...
  if (ability.can('manage', 'all')) return hasAnyVisibleChild
  else return ability.can(item.action, item.resource) && hasAnyVisibleChild
}

export const canViewMenuItem = item => {
  const ability = useContext(AbilityContext)
  if (ability.can('manage', 'all')) return true
  else return ability.can(item.action, item.resource)
}
