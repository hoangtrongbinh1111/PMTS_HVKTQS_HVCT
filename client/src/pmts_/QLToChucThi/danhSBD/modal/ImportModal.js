...
const ImportModal = ({ open, fetchUser, handleModal, listImport, listDiadiem, listDcdt, listTDH, listNganh, listLHDT, listLTN, listCN, listCNH, listDTUT }) => {
    // ** State
    // ** Custom close btn
        ...
        listImport.forEach((row, index) => {
            ...
            if (row[9] !== null) {
                if (!listTDH.find(e => e.kiHieuTruong === row[9])) {
                    temp.push({
                        loi: `Không tìm thấy trường đại học tương ứng với mã "${row[9]}" trong cơ sở dữ liệu`,
            ...
            if (row[10] !== null) {
                if (!listNganh.find(e => e.kihieuNganh === row[10])) {
                    temp.push({
                        loi: `Không tìm thấy ngành đại học tương ứng với mã "${row[10]}" trong cơ sở dữ liệu`,
            ...
            } else if (!listCN.find(e => e.kiHieuChuyennganh === row[16])) {
                temp.push({
                    loi: `Không tìm thấy chuyên ngành đăng kí tuyển sinh tương ứng với mã "${row[16]}" trong cơ sở dữ liệu`,
                    Vitri: `${listColumn[16]}${index + 2}`
                })
            } else {
                infoOneRow.maChuyennganhTS = listCN.find(e => e.kiHieuChuyennganh === row[16]).maChuyennganhTS
            }
            // kiểm tra ngoại ngữ
            if (row[17] === null) {
                temp.push({
