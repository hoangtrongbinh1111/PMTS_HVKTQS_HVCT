...
import { getListChiTieuDetail } from '../../../api/chiTieuTuyenSinh'
...
const DetailDiemChuan = () => {
    ...
    const [chiTieu, setChiTieu] = useState(0)
    const columns = [
        {
            name: "STT",
            center: true,
    ...
    useEffect(() => {
        ...
        getListChiTieuDetail({
            ...
            let tongDS = 0
            temp.map(item => {
                tongQS = tongQS + item.soLuongQS
                tongDS = tongDS + item.soLuongDS
            })
    ...
    const handlOnChangeChuyenNganh = (item) => {
        ...
        if (loaiTS.value === "Quân sự") {
            setSearchValue(item.value.diemChuanQS)
        } else {
            setSearchValue(item.value.diemChuanDS)
        }
    }
    const handlOnChangeLoaiTS = (item) => {
        setLoaiTS({ value: item.value, label: item.label })
        if (item.value === "Quân sự") {
            setSearchValue(chuyenNganh?.value.diemChuanQS)
        } else {
            setSearchValue(chuyenNganh?.value.diemChuanDS)
        }
    }
    ...
    const _handleLuuDiem = async () => {
        ...
        if (loaiTS.value === "Quân sự") {
            data = {
                ...
                body: {
                    diemChuanQS: searchValue
                }
            }
            const temp = Object.assign(chuyenNganh)
            temp.value.diemChuanQS = searchValue
            console.log(temp)
            setChuyenNganh(temp)
        } else {
            data = {
                id: chuyenNganh.value.maChuyennganhTS,
                body: {
                    diemChuanDS: searchValue
                }
            }
            const temp = Object.assign(chuyenNganh)
            temp.value.diemChuanDS = searchValue
            console.log(temp)
            setChuyenNganh(temp)
        }
            ...
            <Card style={{ backgroundColor: 'white' }}>
                    ...
                    <div className='d-flex mt-md-0 mt-1'>
                        <UncontrolledButtonDropdown>
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownItem className='w-100'>
                                    <Printer size={15} />
                                    <span className='align-middle ms-50 ' >Danh sách thí sinh trúng tuyển</span>
                                </DropdownItem>
                            </DropdownMenu>
                        </UncontrolledButtonDropdown>
                ...
                <Row className='justify-content-start mx-0'>
                    <Col className='d-flex align-items-center justify-content-start mt-1' md='12' sm='12' style={{ paddingRight: '20px', paddingLeft: '20px', marginBottom: '20px' }}>
                        {
                            chuyenNganh && <div>
                                <p>Danh sách thí sinh trúng tuyển chuyên ngành <span style={{ fontWeight: 'bold', margin: '5px' }}> {chuyenNganh.label}</span> diện <span style={{ fontWeight: 'bold', margin: '5px' }}> {loaiTS.label}</span> ở mức điểm <span style={{ fontWeight: 'bold', margin: '5px', color: 'red' }}>{searchValue}:</span>  <span style={{ fontWeight: 'bold', margin: '5px', color: 'green' }}>{data.totalCount}</span> thí sinh</p>
                                <p>Chỉ tiêu của chuyên ngành <span style={{ fontWeight: 'bold', margin: '5px' }}> {chuyenNganh.label}</span>diện <span style={{ fontWeight: 'bold', margin: '5px' }}> {loaiTS.label}:</span>  {chiTieu}</p>
                            </div>
                        }
                    </Col>
                </Row>
...
export default DetailDiemChuan
