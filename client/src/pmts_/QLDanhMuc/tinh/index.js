...
// import API
import { getListTinh, searchListTinh, taoTinh, exportTinh, exportTinhTemplate } from '../../../api/tinh'
 ...
 * 1. Tinh: Trường đại học
 */
const Tinh = () => {
    const inputFile = useRef(null)
    const ability = useContext(AbilityContext)
    // ** States
    const [modalThemTinh, setModalThemTinh] = useState(false)
    const [modalSuaTinh, setModalSuaTinh] = useState(false)
    const [modalXoaTinh, setModalXoaTinh] = useState(false)
    ...
    const _handleSuaTinh = (data) => {
        setInfo(data)
        setModalSuaTinh(true)
    }
    const _handleXoaTinh = (data) => {
        setInfo(data)
        setModalXoaTinh(true)
    }
    const columns = [
        {
            name: 'STT',
            width: '70px',
            cell: (row, index) => <span>{((currentPage * perPage) + index + 1)}</span>
        },
        {
            name: 'Tên Tỉnh',
            minWidth: '250px',
            selector: row => row.tenTinh,
            ...
            cell: (row) => {
                return (
                    <div className='d-flex' style={{ padding: '4px' }}>
                        {ability.can('update', 'tinh') &&
                            <Edit size={15} onClick={e => _handleSuaTinh(row)} style={{ cursor: 'pointer', color: 'green', marginRight: '10px' }} />}
                        {ability.can('delete', 'tinh') &&
                            <Trash size={15} onClick={e => _handleXoaTinh(row)} style={{ cursor: 'pointer', color: 'red' }} />}
                    </div>
                )
            }
        }
    ]
    // ** Function to handle modalThemTinh toggle
    const handleModalThemTinh = () => setModalThemTinh(!modalThemTinh)
    const handleModalSuaTinh = () => setModalSuaTinh(!modalSuaTinh)
    const handleModalXoaTinh = () => setModalXoaTinh(!modalXoaTinh)
    const fetchUser = () => {
        getListTinh({
            page: currentPage + 1,
            perPage
    ...
    const handleExportFileExcel = async () => {
        const res = await exportTinh()
    }
    const handleExportFileTemplate = async () => {
        const res = await exportTinhTemplate()
    }
        ...
        <Fragment>
            ...
            <Card style={{ backgroundColor: 'white' }}>
                <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                    <CardTitle tag='h4'>Danh sách tỉnh</CardTitle>
                    <div className='d-flex mt-md-0 mt-1'>
                        ...
                        {ability.can('add', 'tinh') &&
                            <Button className='ms-2' color='primary' onClick={handleModalThemTinh}>
                                <Plus size={15} />
            ...
            <AddNewModal open={modalThemTinh} handleModal={handleModalThemTinh} fetchUser={fetchUser} />
            {
                <EditModal open={modalSuaTinh} handleModal={handleModalSuaTinh} fetchUser={fetchUser} infoEdit={info} />
            }
            {
                <DeleteModal open={modalXoaTinh} handleModal={handleModalXoaTinh} fetchUser={fetchUser} infoEdit={info} />
            }
        </Fragment>
    )
}
