...
// import API
import { getListHinhThucKyLuat, searchListHinhThucKyLuat, taoHinhThucKyLuat, exportHinhThucKyLuat, exportHinhThucKyLuatTemplate } from '../../../api/hinhThucKyLuat'
 ...
 * 1. HTKL: Hình thức Kỷ Luật
 */
const HinhThucKyLuat = () => {
    const ability = useContext(AbilityContext)
    const inputFile = useRef(null)
    // ** States
    const [loading, setLoading] = useState(true)
    const [modalThemHTKL, setModalThemHTKL] = useState(false)
    const [modalSuaHTKL, setModalSuaHTKL] = useState(false)
    const [modalXoaHTKL, setModalXoaHTKL] = useState(false)
    ...
    const _handleSuaHTKL = (data) => {
        setInfo(data)
        setModalSuaHTKL(true)
    }
    const _handleXoaHTKL = (data) => {
        setInfo(data)
        setModalXoaHTKL(true)
    }
    const columns = [
        {
            name: 'STT',
            ...
            cell: (row) => {
                return (
                    <div className='d-flex' style={{ padding: '4px' }}>
                        {ability.can('update', 'hinhthuckiluat') &&
                            <Edit size={15} onClick={e => _handleSuaHTKL(row)} style={{ cursor: 'pointer', color: 'green', marginRight: '10px' }} />}
                        {ability.can('delete', 'hinhthuckiluat') &&
                            <Trash size={15} onClick={e => _handleXoaHTKL(row)} style={{ cursor: 'pointer', color: 'red' }} />}
                    </div>
                )
            }
        }
    ]
    // ** Function to handle modalThemHTKL toggle
    const handleModalThemHTKL = () => setModalThemHTKL(!modalThemHTKL)
    const handleModalSuaHTKL = () => setModalSuaHTKL(!modalSuaHTKL)
    const handleModalXoaHTKL = () => setModalXoaHTKL(!modalXoaHTKL)
    const fetchUser = () => {
        searchListHinhThucKyLuat({
            page: currentPage,
            perPage,
    ...
    const handleExportFileExcel = async () => {
        const res = await exportHinhThucKyLuat()
    }
    const handleExportFileTemplate = async () => {
        const res = await exportHinhThucKyLuatTemplate()
    }
        ...
        <Fragment>
            ...
            <Card style={{ backgroundColor: 'white' }}>
                <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                    <CardTitle tag='h4'>Danh sách hình thức kỷ luật</CardTitle>
                    <div className='d-flex mt-md-0 mt-1'>
                        <UncontrolledButtonDropdown>
                            ...
                            <DropdownMenu>
                                {ability.can('add', 'hinhthuckiluat') &&
                        ...
                        {ability.can('add', 'hinhthuckiluat') &&
                            <Button className='ms-2' color='primary' onClick={handleModalThemHTKL}>
                                <Plus size={15} />
            ...
            <AddNewModal open={modalThemHTKL} handleModal={handleModalThemHTKL} fetchUser={fetchUser} />
            {
                <EditModal open={modalSuaHTKL} handleModal={handleModalSuaHTKL} fetchUser={fetchUser} infoEdit={info} />
            }
            {
                <DeleteModal open={modalXoaHTKL} handleModal={handleModalXoaHTKL} fetchUser={fetchUser} infoEdit={info} />
            }
        </Fragment>
    )
}
