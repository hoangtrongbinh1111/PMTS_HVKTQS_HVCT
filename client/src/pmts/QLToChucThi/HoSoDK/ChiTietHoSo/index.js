...
const ChiTietHoSo = () => {
    ...
    const fetchDetail = async () => {
        getChiTietHoSo({ id: params.id }).then(res => {
            setInfoHoso(res.result.data)
            const url = process.env.REACT_APP_API_URL_NEW
            if (res.result.data.fileAnh) {
                axios.get(`${url}/public/images/${localStorage.getItem('dbName')}/${res.result.data.fileAnh}`).then(respnsive => {
                    ...
                    <Row>
                        <Row>
                            <Col md='3' sm='12' >
                                ...
                                {
                                    edit === true ? <></> : <Input type='file' onChange={e => handleOnChangeFile(e)}></Input>
                                }
                            </Col>
                            <Col md='9' sm='12' >
                                <Row>
