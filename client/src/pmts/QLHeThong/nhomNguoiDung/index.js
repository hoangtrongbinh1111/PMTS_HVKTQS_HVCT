...
const NhomNguoiDung = () => {
            ...
            cell: (row) => {
                return (
                    <div className='d-flex' style={{ padding: '4px' }}>
                                                {ability.can('update', 'nhomnguoidung') &&
                        <Edit size={15} onClick={e => _handleSuaNND(row)} style={{ cursor: 'pointer', color: 'green', marginRight: '10px' }} />}
                        {ability.can('delete', 'nhomnguoidung') &&
                        <Trash size={15} onClick={e => _handleXoaNND(row)} style={{ cursor: 'pointer', color: 'red' }} />}
                    </div>
                )
            }
        }
    ...
    const handleExportFileExcel = async (data) => {
        const res = await exportGroupUser(data)
        const mydata = await fetch("http://localhost:4001/output/output.xlsx")
        const templateFile = await mydata.blob()
        const file = new Blob([templateFile])
        saveFile("danhsachnhomnguoidung.xlsx", file)
    }
    const handleExportFileTemplate = async () => {
        const mydata = await fetch("http://localhost:4001/public/template/filemau.xlsx")
        const templateFile = await mydata.blob()
        const file = new Blob([templateFile])
        saveFile("Nhập nhóm người dùng mẫu.xlsx", file)
    }
    ...
    const handleExportFileDocx = async (data) => {
        const request = await fetch("/../../template/dshoso.docx")
    ...
    return (
        <Fragment>
            ...
            {
                <EditModal open={modalSuaNND} handleModal={handleModalSuaNND} fetchUser={fetchUser} infoEdit={info} />
            }
            {
                <DeleteModal open={modalXoaNND} handleModal={handleModalXoaNND} fetchUser={fetchUser} infoEdit={info} />
            }
        </Fragment>
    )
}
