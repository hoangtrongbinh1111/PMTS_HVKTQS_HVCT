...
// import API
import { getListDiaChiDaoTao, searchListDiaChiDaoTao, taoDiaChiDaoTao, exportDiaChiDaoTao, exportDiaChiDaoTaoTemplate } from '../../../api/diaChiDaoTao'
...
const DiaChiDaoTao = () => {
    ...
    // ** States
    const [modalThemDCDT, setModalThemDCDT] = useState(false)
    const [modalSuaDCDT, setModalSuaDCDT] = useState(false)
    const [modalXoaDCDT, setModalXoaDCDT] = useState(false)
    ...
    const _handleSuaDCDT = (data) => {
        setInfo(data)
        setModalSuaDCDT(true)
    }
    const _handleXoaDCDT = (data) => {
        setInfo(data)
        setModalXoaDCDT(true)
    }
    const columns = [
        {
            name: 'STT',
        ...
        {
            name: 'Tên Địa Chỉ Đào Tạo',
            minWidth: '250px',
            selector: row => row.tenDc,
        },
        {
            name: 'Kí hiệu',
            sortable: true,
            ...
            cell: (row) => {
                return (
                    <div className='d-flex' style={{ padding: '4px' }}>
                        {ability.can('update', 'diachidaotao') &&
                            <Edit size={15} onClick={e => _handleSuaDCDT(row)} style={{ cursor: 'pointer', color: 'green', marginRight: '10px' }} />}
                        {ability.can('delete', 'diachidaotao') &&
                            <Trash size={15} onClick={e => _handleXoaDCDT(row)} style={{ cursor: 'pointer', color: 'red' }} />}
                    </div>
                )
            }
        }
    ]
    // ** Function to handle modalThemDCDT toggle
    const handleModalThemDCDT = () => setModalThemDCDT(!modalThemDCDT)
    const handleModalSuaDCDT = () => setModalSuaDCDT(!modalSuaDCDT)
    const handleModalXoaDCDT = () => setModalXoaDCDT(!modalXoaDCDT)
    const fetchUser = () => {
        searchListDiaChiDaoTao({
            page: currentPage,
            perPage,
            tenDc: searchValue
        }).then(res => {
    ...
    const handleExportFileExcel = async () => {
        const res = await exportDiaChiDaoTao()
    }
    const handleExportFileTemplate = async () => {
        const res = await exportDiaChiDaoTaoTemplate()
    }
        ...
        <Fragment>
            ...
            <Card style={{ backgroundColor: 'white' }}>
                <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                    <CardTitle tag='h4'>Danh sách địa chỉ đào tạo</CardTitle>
                    <div className='d-flex mt-md-0 mt-1'>
                        ...
                        {ability.can('add', 'diachidaotao') &&
                            <Button className='ms-2' color='primary' onClick={handleModalThemDCDT}>
                                <Plus size={15} />
            ...
            <AddNewModal open={modalThemDCDT} handleModal={handleModalThemDCDT} fetchUser={fetchUser} />
            {
                <EditModal open={modalSuaDCDT} handleModal={handleModalSuaDCDT} fetchUser={fetchUser} infoEdit={info} />
            }
            {
                <DeleteModal open={modalXoaDCDT} handleModal={handleModalXoaDCDT} fetchUser={fetchUser} infoEdit={info} />
            }
        </Fragment>
    )
}
