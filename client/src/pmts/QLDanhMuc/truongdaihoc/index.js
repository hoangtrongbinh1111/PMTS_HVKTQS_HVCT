...
// import API
import { getListTruongDaiHoc, searchListTruongDaiHoc, taoTruongDaiHoc, exportTruongDaiHoc, exportTruongDaiHocTemplate } from '../../../api/truongDaiHoc'
...
const TruongDaiHoc = () => {
    ...
    // ** States
    const [modalThemTDH, setModalThemTDH] = useState(false)
    const [modalSuaTDH, setModalSuaTDH] = useState(false)
    const [modalXoaTDH, setModalXoaTDH] = useState(false)
    ...
    const _handleSuaTDH = (data) => {
        setInfo(data)
        setModalSuaTDH(true)
    }
    const _handleXoaTDH = (data) => {
        setInfo(data)
        setModalXoaTDH(true)
    }
    const columns = [
        ...
        },
        {
            name: 'Tên Trường',
            minWidth: '250px',
            selector: row => row.tenTruong,

        },
        {
            name: 'Kí hiệu',
            minWidth: '250px',
            cell: (row) => <span style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%' }}>{(row.kiHieuTruong)}</span>
            ...
            cell: (row) => {
                return (
                    <div className='d-flex' style={{ padding: '4px' }}>
                        {ability.can('update', 'truongdaihoc') &&
                            <Edit size={15} onClick={e => _handleSuaTDH(row)} style={{ cursor: 'pointer', color: 'green', marginRight: '10px' }} />}
                        {ability.can('delete', 'truongdaihoc') &&
                            <Trash size={15} onClick={e => _handleXoaTDH(row)} style={{ cursor: 'pointer', color: 'red' }} />}
                    </div>
                )
            }
        }
    ]
    // ** Function to handle modalThemTDH toggle
    const handleModalThemTDH = () => setModalThemTDH(!modalThemTDH)
    const handleModalSuaTDH = () => setModalSuaTDH(!modalSuaTDH)
    const handleModalXoaTDH = () => setModalXoaTDH(!modalXoaTDH)
    const fetchUser = () => {
        ...
        <Fragment>
            ...
            <Card style={{ backgroundColor: 'white' }}>
                <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                    <CardTitle tag='h4'>Danh sách trường đại học</CardTitle>
                    <div className='d-flex mt-md-0 mt-1'>
                        ...
                        {ability.can('add', 'truongdaihoc') &&
                            <Button className='ms-2' color='primary' onClick={handleModalThemTDH}>
                                <Plus size={15} />
            ...
            <AddNewModal open={modalThemTDH} handleModal={handleModalThemTDH} fetchUser={fetchUser} />
            {
                <EditModal open={modalSuaTDH} handleModal={handleModalSuaTDH} fetchUser={fetchUser} infoEdit={info} />
            }
            {
                <DeleteModal open={modalXoaTDH} handleModal={handleModalXoaTDH} fetchUser={fetchUser} infoEdit={info} />
            }
        </Fragment>
    )
}
