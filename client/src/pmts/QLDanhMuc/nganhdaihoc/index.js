...
const NganhDaiHoc = () => {
    ...
    // ** States
    const [modalThemNDH, setModalThemNDH] = useState(false)
    const [modalSuaNDH, setModalSuaNDH] = useState(false)
    const [modalXoaNDH, setModalXoaNDH] = useState(false)
    ...
    const _handleSuaNDH = (data) => {
        setInfo(data)
        setModalSuaNDH(true)
    }
    const _handleXoaNDH = (data) => {
        setInfo(data)
        setModalXoaNDH(true)
    }
    const columns = [
        ...
        },
        {
            ...
            sortable: true,
            minWidth: '250px',
            cell: (row) => <span style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%' }}>{(row.kihieuNganh)}</span>
        },
        {
            name: 'Ghi chú',
            sortable: true,
            ...
            cell: (row) => {
                return (
                    <div className='d-flex' style={{ padding: '4px' }}>
                        {ability.can('update', 'nganh') &&
                            <Edit size={15} onClick={e => _handleSuaNDH(row)} style={{ cursor: 'pointer', color: 'green', marginRight: '10px' }} />}
                        {ability.can('delete', 'nganh') &&
                            <Trash size={15} onClick={e => _handleXoaNDH(row)} style={{ cursor: 'pointer', color: 'red' }} />}
                    </div>
                )
            }
        }
    ]
    // ** Function to handle modalThemNDH toggle
    const handleModalThemNDH = () => setModalThemNDH(!modalThemNDH)
    const handleModalSuaNDH = () => setModalSuaNDH(!modalSuaNDH)
    const handleModalXoaNDH = () => setModalXoaNDH(!modalXoaNDH)
    const fetchUser = () => {
        searchListNganhDaiHoc({
            page: currentPage,
            perPage,
    ...
    const handleExportFileExcel = async () => {
        const res = await exportNganhDaiHoc()
    }
    const handleExportFileTemplate = async () => {
        const res = await exportNganhDaiHocTemplate()
    }
    ...
    const handleExportFileDocx = async (data) => {
        const request = await fetch("/../../template/nganhdaihoc/dsnganhdaihoc.docx")
        const templateFile = await request.blob()
        const handler = new TemplateHandler()
        const docx = await handler.process(templateFile, data)
        saveFile("dsnganhdaihoc.docx", docx)
    }
    return (
        <Fragment>
            ...
            <Card style={{ backgroundColor: 'white' }}>
                <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                    <CardTitle tag='h4'>Danh sách ngành đại học</CardTitle>
                    <div className='d-flex mt-md-0 mt-1'>
                        ...
                        {ability.can('add', 'nganh') &&
                            <Button className='ms-2' color='primary' onClick={handleModalThemNDH}>
                                <Plus size={15} />
            ...
            <AddNewModal open={modalThemNDH} handleModal={handleModalThemNDH} fetchUser={fetchUser} />
            {
                <EditModal open={modalSuaNDH} handleModal={handleModalSuaNDH} fetchUser={fetchUser} infoEdit={info} />
            }
            {
                <DeleteModal open={modalXoaNDH} handleModal={handleModalXoaNDH} fetchUser={fetchUser} infoEdit={info} />
            }
        </Fragment>
    )
}
