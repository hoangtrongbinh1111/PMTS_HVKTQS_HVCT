...
import { xoaChuyenNganh } from '../../../../api/chuyenNganh'
...
const DeleteModal = ({ open, handleModal, fetchUser, infoEdit }) => {
  ...
  const _handleXoaNMT = async () => {
    setLoading(true)
    const res = await xoaChuyenNganh({
  ...
  return (
    ...
    >
      <ModalHeader className='mb-1' toggle={handleModal} close={CloseBtn} tag='div'>
        <h5 className='modal-title'>Xóa chuyên ngành tuyển sinh</h5>
      </ModalHeader>
      <ModalBody className='flex-grow-1'>
        <div className='mb-1'>
          <Label className='form-label' for='full-name'>
            Bạn có chắc chắn xóa <span style={{ fontWeight: "bold" }}>{infoEdit?.tenChuyennganh}</span>
          </Label>
        </div>
