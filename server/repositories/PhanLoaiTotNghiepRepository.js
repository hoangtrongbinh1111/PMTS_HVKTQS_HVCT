...
const { TBL_PLTOTNGHIEPDH, TBL_PLTOTNGHIEPDH_KEY } = require("../schemas/constant");
const { getSlaveDatabase } = require('../db');

class LoaiHinhDaoTaoRepository extends BaseRepository {
  constructor() {
    super(`${TBL_PLTOTNGHIEPDH}`, `${TBL_PLTOTNGHIEPDH_KEY}`, getSlaveDatabase(), knex); // Specify the table name for the UserRepository
  }
  ...
  async getAll(page, perPage, query = '') {
    if (query) {
      ...
      queryBuilder = queryBuilder.where((builder) => {
        ...
        builder.where('tenPhanloai', 'like', `%${query}%`);
        builder.orWhere('kiHieuPhanloai', 'like', `%${query}%`);
  ...
  async _getTotalCountAndLastRow(queryBuilder) {
    ...
    const totalCount = countResult ? countResult['count(`maPhanloai`)'] : 0;
    return {
      countResult,
      totalCount
    };
