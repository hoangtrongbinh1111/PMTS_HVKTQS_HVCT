...
const { TBL_HTKYLUAT, TBL_HTKYLUAT_KEY } = require("../schemas/constant");
const { getSlaveDatabase } = require('../db');

class HinhThucKyLuatRepository extends BaseRepository {
  constructor() {
    super(`${TBL_HTKYLUAT}`, `${TBL_HTKYLUAT_KEY}`, getSlaveDatabase(), knex); // Specify the table name for the UserRepository
  }
  ...
  async getAll(page, perPage, query = '') {
    if (query) {
      ...
      queryBuilder = queryBuilder.where((builder) => {
        ...
        builder.where('tenHinhthuc', 'like', `%${query}%`);
        builder.orWhere('mucTru', 'like', `%${query}%`);
      });
...
module.exports = HinhThucKyLuatRepository;
